...
class TellTimeAgent:
    ...
    def _build_agent(self) -> LlmAgent:
        ...
        return LlmAgent(
            ...
            model=LiteLlm(model="openai/gpt-4o"),    # Specify Gemini model version         (가능)
            name="tell_time_agent",                  # Name of the agent
            description="Tells the current time",    # Description for metadata
            instruction="""Follow these steps to get the current time and reply:
                            1. Use the tool(_get_current_time()) to get current time. 
                            2. Reply with the current time in the format YYYY-MM-DD HH:MM:SS.""",  # System prompt
            tools=[
                self._get_current_time,  # Function to get current time
